...
TFT_eSPI aicam = TFT_eSPI();

class MjpegClass
{
public:
  bool setup(File input, uint8_t *mjpeg_buf, bool multiTask)
  {
    _input = input;
    _mjpeg_buf = mjpeg_buf;
    _multiTask = multiTask;

    _tft_width = aicam.width();
    _tft_height = aicam.height();
...
private:
  ...
  static uint32_t jpgWriteRow(TJpgD *jdec, uint32_t y, uint32_t h)
  {
    ...
    if (y == 0)
    {
      aicam.setAddrWindow(me->_jpg_x, me->_jpg_y, jdec->width, jdec->height);
    }

    aicam.startWrite();
    aicam.pushColors((uint8_t *)me->_out_buf, jdec->width * h * 2);
    aicam.endWrite();
